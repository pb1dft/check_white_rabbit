...
def check_sfp(host, community):
    ...
    for i, pname in enumerate(port_names):
        ...
        #vendor_val = vendor_val.strip().strip('"')
    ...
    if status_val == 1 and not problems:
        print(f"OK: All SFPs OK | " + " ".join(perfdata))
        sys.exit(0)
...
def main():
    ...
    if args.mode == "cpu":
        ...
        numeric_vals = {k: float(snmp_get(args.host, args.community, oid)) for k, oid in CPU_NUMERIC_OIDS.items()}
        perfdata = " ".join([f"{k}={v}" for k,v in numeric_vals.items()])

        print(f"{status_str}: CPU Load {numeric_vals['cpu1']}/{numeric_vals['cpu5']}/{numeric_vals['cpu15']} | {perfdata}")
        sys.exit(exit_code)
    ...
    if args.mode == "temp":
        ...
        temps = {k:int(snmp_get(args.host,args.community,oid)) for k,oid in TEMP_VALUE_OIDS.items()}
        thresholds = {k:int(snmp_get(args.host,args.community,oid)) for k,oid in TEMP_THRESH_OIDS.items()}
    ...
    if args.mode == "mem":
        ...
        memvals = {k:int(snmp_get(args.host,args.community,oid)) for k,oid in MEM_VALUE_OIDS.items()}
        perf = " ".join([f"{k}={v}" for k,v in memvals.items()])

        print(f"{msg}: Used {memvals['used']} / {memvals['total']} ({memvals['usedPerc']}%) | {perf}")
        sys.exit(code)
